...
describe("recruiter engagement composition", () => {
  it("keeps a run on the execution settings it started with when Settings change mid-run", async () => {
    const { recruiterEngagementWeb } = await import("./recruiter-engagement-web.server");
    recruiterEngagementWeb.saveExecutionSettings({ ...frozen, stageRequestLimit: 2 });

    const started = await recruiterEngagementWeb.startResearchRun({
    ...
    recruiterEngagementWeb.saveExecutionSettings({ ...changed, stageRequestLimit: 2 });
    expect(recruiterEngagementWeb.getExecutionSettings().model).toBe(changed.model);
    writeFileSync(release, "");

    await until(async () => {
      const research = await recruiterEngagementWeb.getResearchRun(started.runId);
