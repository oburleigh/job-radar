...
export function getDashboardData(filters: JobFilters = {}) {
  ...
  const matchedRows = db
    ...
    .leftJoin(
      jobStates,
      and(eq(jobStates.jobId, jobs.id), eq(jobStates.profileId, profile.id)),
    )
    .where(
      and(
  ...
  const activeRows = dedupeCrossSourceMatches(
    matchedRows.filter((row) => row.state !== "hidden"),
  );
  const rows =
    filters.state === "hidden"
      ? dedupeCrossSourceMatches(
          matchedRows.filter((row) => row.state === "hidden"),
        )
      : activeRows;
  ...
  const filteredJobs = rows
    ...
    .filter(
      (row) =>
        !query ||
        `${row.title} ${row.companyName} ${row.locationText}`
          .toLowerCase()
          .includes(query),
    )
    .slice(0, 250);
...
function formatPublishedSalary(
  description: string,
...
>(rows: T[]): T[] {
  ...
  for (const row of rows) {
    const key = `${normalizeDedupeText(row.companyName)}|${normalizeDedupeText(
      row.title,
    )}`;
    ...
    if (
      existing &&
      existing.atsType === "linkedin" &&
      row.atsType !== "linkedin"
    ) {
      result[position] = row;
    }
  }
...
export function getSourcesData() {
  return {
    sources: db
      .select()
      .from(sourceDomains)
      .orderBy(sourceDomains.priority)
      .all(),
    boards: db
      .select({
        id: companyBoards.id,
