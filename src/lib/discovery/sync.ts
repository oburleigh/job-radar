...
import { companyBoards, jobs, searchProfiles } from "@/db/schema";

import { fetchBoardJobs } from "./connectors";
import { evaluateAndStore } from "./store-matches";
import type { AtsType, BoardInput, RawJob } from "./types";
...
export async function syncBoard(
  board: BoardInput,
  limit = getJobRadarConfig().discovery.boardJobLimit,
): Promise<SyncResult> {
  let created = 0;
  let updated = 0;
