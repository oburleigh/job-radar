...
import type { AtsType, RawJob } from "./types";

const STRUCTURED_JOB_SOURCES = new Set(["web3-career", "cryptocurrencyjobs", "cryptojobslist"]);
...
export function supportsStructuredJobPage(atsType: AtsType): boolean {
  return STRUCTURED_JOB_SOURCES.has(atsType);
}
...
export async function fetchStructuredJobPage(
  ...
  if (CLOSED_MARKERS.some((marker) => normalizedPage.includes(marker))) {
